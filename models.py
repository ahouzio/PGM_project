        ...
        with torch.no_grad():
            for i, sigma in enumerate(sigmas):
                ...
                for k in range(n_steps + 1):
                    ...
                    x_k += alpha * score.detach() + np.sqrt(
                        2 * alpha
                    ) * torch.randn_like(
                        x_k
...
class SimpleNoiseConditionalScoreNetwork(nn.Module):
    def __init__(
        self, hidden_dim: int = 128, data_dim: int = 2, num_sigmas: int = 5
    ) -> None:
        super().__init__()
        ...
        with torch.no_grad():
            for i, sigma in enumerate(sigmas):
                ...
                for k in range(n_steps):
                    ...
                    score = self(x_k, sigma_batch)
                    x_k += alpha * score.detach() / 2 + np.sqrt(
                        alpha
                    ) * torch.randn_like(x_k)
                    if save_history and k % save_freq == 0:
                        history.append(x_k.cpu())
    ...
    def Langevin(
        ...
        sigma_batch = (
            torch.ones(n_samples, dtype=torch.int, device=x_k.device) * sigma_idx
        )
        with torch.no_grad():
            for k in range(n_steps):
                score = self(x_k, sigma_batch)
